...
- **The header bell already exists:** [src/components/Header.tsx](../../../src/components/Header.tsx) renders a bell + dropdown + unread badge fed by localStorage notices ([src/lib/notifications.ts](../../../src/lib/notifications.ts), kinds `live | account`, `ca:notices`, mark-all-read on open). Forum notifications merge into it — no second bell.
...
| [Header.tsx](../../../src/components/Header.tsx) bell | Gated on `currentKickUser()`; uses `useMe` (shared fetch, also self-heals the forum session) for `kick_id`. Badge = local unread + forum unread. Dropdown merges both sources sorted by time; opening marks **both** stores read (current behavior preserved). Forum items navigate on click. |
